...
Modelica.Blocks.Sources.RealExpression[3] realExpression1(y = if time > 10 and time < 15 then {del[1] + 3.1412 / 180, 0, 0} else del)  annotation(    Placement(visible = true, transformation(origin = {-142, -5}, extent = {{-26, -47}, {26, 47}}, rotation = 0)));
      Flight_Dynamics.Components.ForceMoment_Gen forceMoment_Gen1 annotation(
...
equation
      ...
      connect(realExpression1.y, forceMoment_Gen1.delta) annotation(
        Line(points = {{-113, -5}, {-50, -5}, {-50, -4}, {-48, -4}}, color = {0, 0, 127}, thickness = 0.5));
      forceMoment_Gen1.thrust = thrust;
      annotation(experiment(StartTime = 0, StopTime = 300, Tolerance = 1e-3, Interval = 0.001),
    uses(Modelica(version = "3.2.2")));
    end CessnaPerturbed;

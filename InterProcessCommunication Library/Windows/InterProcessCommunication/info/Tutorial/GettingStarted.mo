...
4). Go to the directory (Windows/InterProcessCommunication/Resources/Include/) and open file Serial_SHM.c.<br /> 
5). Check for the port name given to the serial device by your system by using Device Manager.<br /> 
6). Create executable for Serial_SHM.c using any C compiler and run its executable.<br />
7). Fill in the device ID (e.g. COM5) and baud rate (e.g. 115200) when prompted to do so. It will establish the connection. Keep it running in the background.<br /> 
8). Go to simulation setup in OpenModelica and change Stop Time to a value equal to or greater than 30 on both PC’s. <br />
9). Click on Simulation Flags tab and write -rt=1 in column for Additional Simulation Flags and simulate on both PC’s simultaneously.
</p>

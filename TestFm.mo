...
parameter  Real del[3] = {0,-0.045745,0};
parameter  Real thrust[3] = {-0.442306 , 0, 0};
parameter Real alphazero = -0.0149684;
  ...
  Flight6DOF flight6DOF1(J = {{0.1147, 0, -0.0015}, {0, 0.0576, 0}, {-0.0015, 0, 0.1712}}, g = {0,0, 9.81},mass = m, omega( fixed = true,start = {0, 0, 0}), pos(start = {0, 0, -1000}), v(start = {60*cos(alphazero), 0, 60*sin(alphazero)}))  annotation(
    Placement(visible = true, transformation(origin = {47, -1}, extent = {{-13, -13}, {13, 13}}, rotation = 0)));
  
//Modelica.Blocks.Sources.RealExpression[3] realExpression1(y = if time > 10 and time < 11 then {0, -0.01304370748, 0} else del)  annotation(    Placement(visible = true, transformation(origin = {-82, -5}, extent = {{-26, -9}, {26, 9}}, rotation = 0)));
initial equation
forceMoment_Gen1.alpha = alphazero;
forceMoment_Gen1.angles[2] = alphazero;
equation
  forceMoment_Gen1.delta = del;
 //connect(forceMoment_Gen1.delta, realExpression1.y) annotation(Line(points = {{-28, -4}, {-41.5, -4}, {-41.5, -5}, {-53, -5}}, color = {0, 0, 127}, thickness = 0.5));
  ...
  connect(flight6DOF1.v, forceMoment_Gen1.vel) annotation(
    Line(points = {{61, 11}, {90, 11}, {90, 42}, {-58, 42}, {-58, 20}, {-9, 20}}, color = {0, 0, 127}, thickness = 0.5));
  ...
  connect(flight6DOF1.angles, forceMoment_Gen1.angles) annotation(
    Line(points = {{61, -13}, {102, -13}, {102, 54}, {-70, 54}, {-70, 20}, {-17.5, 20}}, color = {0, 0, 127}, thickness = 0.5));

...
Modelica.Blocks.Sources.RealExpression deltaE (y = if time > 100 and time < 105 then  -0.15625+3.1412 /180 else -0.15625)  annotation(    Placement(visible = true, transformation(origin = {-112, 1}, extent = {{-26, -47}, {26, 47}}, rotation = 0)));
...
equation
CL = CL0+CL_alpha*alpha+((CL_q*q*C_bar)/(2*V))+CL_delta_e*deltaE.y;
...
Cm  = Cm0+Cm_alpha*alpha+((Cm_q*q*C_bar)/(2*V))+Cm_delta_e*deltaE.y;//pitching coeff

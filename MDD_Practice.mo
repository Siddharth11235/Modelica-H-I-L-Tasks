package MDD_Practice
  ...
  model avr_test
    ...
    Modelica_DeviceDrivers.Blocks.Communication.SerialPortReceive serialPortReceive1(Serial_Port = "/dev/ttyACM0", autoBufferSize = true, baud = Modelica_DeviceDrivers.Utilities.Types.SerialBaudRate.B9600) annotation(
      Placement(visible = true, transformation(origin = {38, 32}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
    Modelica_DeviceDrivers.Blocks.Packaging.SerialPackager.GetFloat getFloat1 annotation(
    ...
    inner Modelica_DeviceDrivers.EmbeddedTargets.AVR.Blocks.Microcontroller mcu(desiredPeriod = 0.1, platform = Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.Platform.ATmega328P) annotation(
      Placement(visible = true, transformation(origin = {-74, 66}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica_DeviceDrivers.EmbeddedTargets.AVR.Blocks.PWM pwm(fastPWM = true, initialValues = 0, prescaler = Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.TimerPrescaler.'1/128', timer = Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.TimerSelect.Timer0, timerNumbers = Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.TimerNumber.B) annotation(
      Placement(visible = true, transformation(origin = {-38, 30}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    Modelica.Blocks.Sources.IntegerExpression integerExpression1(y = integer(3 * sin(time))) annotation(
  ...
  equation
    connect(integerExpression1.y, pwm.u[1]) annotation(
      Line(points = {{-70, 30}, {-50, 30}, {-50, 30}, {-50, 30}}, color = {255, 127, 0}));
    connect(getFloat1.y[1], realValue1.numberPort) annotation(
      Line(points = {{50, -14}, {60, -14}, {60, -14}, {60, -14}}, color = {0, 0, 127}, thickness = 0.5));
    connect(serialPortReceive1.pkgOut, getFloat1.pkgIn) annotation(
      Line(points = {{38, 22}, {38, 22}, {38, -4}, {38, -4}}));
    annotation(
  ...
  class avr_io
    import Modelica_DeviceDrivers.*;
    Modelica_DeviceDrivers.Blocks.Communication.SerialPortSend serialSend(Serial_Port = "/dev/ttyACM0", autoBufferSize = true, baud = Modelica_DeviceDrivers.Utilities.Types.SerialBaudRate.B115200, sampleTime = 0.005) annotation(
    ...
    Modelica_DeviceDrivers.Blocks.Communication.SerialPortReceive serialReceive(Serial_Port = "/dev/ttyACM0", autoBufferSize = true, baud = Modelica_DeviceDrivers.Utilities.Types.SerialBaudRate.B115200, sampleTime = 0.02, startTime = 0) annotation(
